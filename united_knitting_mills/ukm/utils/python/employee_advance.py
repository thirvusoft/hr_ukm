...
def advance_validation(employee):
    ...
    if latest_salary_slip:
        ...
        if emp_shift_amount and previous_advance!=0:
...
def validate(doc,event):
    if doc.advance_amount and not doc.advance_amount==0:
        if doc.eligible_amount and not doc.eligible_amount==0 and not doc.eligible_amount<0:
