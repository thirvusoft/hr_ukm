...
frappe.ui.form.on('Attendance',{
    onload: function(frm) {
		if (frm.doc.docstatus != 1 && frm.doc.docstatus != 2 && !frm.doc.__islocal){
			cur_frm.set_value('checkin_details',[])
			frappe.db.get_list("Employee Checkin", {filters: {"attendance": frm.doc.name}, fields: ["time", "log_type"]}).then((list) => {
