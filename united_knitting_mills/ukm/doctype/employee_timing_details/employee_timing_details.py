...
def check_break_time_checkins_for_staff(reason, att_name, doc, submit_doc, times):
...
def validate_total_working_hours(reason, doc, submit_doc, hours_to_work, checkins, attendance):
	...
	if(not attendance.thirvu_shift_details[0].get('start_time') or not attendance.thirvu_shift_details[0].get('end_time')):
		...
		submit_doc, reason = check_break_time_checkins_for_staff(reason, attendance.name, doc, submit_doc, times)
		for time in times:
			if(len(time) == 2):
		...
		if(worked_time <working_hours):
			reason +=f"\n-> Insufficient Working Hours({working_hours} hrs required but only {worked_time} worked)."
			submit_doc = False
