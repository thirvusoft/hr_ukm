...
class ShiftType(Document):
	...
	def process_auto_attendance(self):
		...
		# logs=frappe.db.sql('''select * from `tabEmployee Checkin` where skip_auto_attendance=0 and attendance ={0} and time >= {1} and shift_actual_end < {2} and shift={3}'''.format(('is','not set'),self.process_attendance_after,self.last_sync_of_checkin,self.name),as_list=1)
