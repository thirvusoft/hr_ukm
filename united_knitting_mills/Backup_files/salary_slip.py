...
#------------------------------------------------------
	#One Half Day
		fraction_of_daily_salary_for_three_quarter_day = (
			flt(frappe.db.get_value("Payroll Settings", None, "fraction_of_daily_salary_for_one_half_day")) or 1.50
		)
#------------------------------------------------------
...
# 					"""
# 					SELECT t1.name,
# 						CASE WHEN (t1.one_half_day_date = %(dt)s or t1.to_date = t1.from_date)
# 						THEN t1.one_half_day else 0 END,
# 						t2.is_ppl,
# 						t2.fraction_of_daily_salary_per_leave
# 					FROM `tabLeave Application` t1, `tabLeave Type` t2
# 					WHERE t2.name = t1.leave_type
...
# 				if leave:
# 					equivalent_lwp_count = 0
# 					is_one_half_day_leave = cint(leave[0][1])
# 					is_partially_paid_leave = cint(leave[0][2])
# 					fraction_of_daily_salary_per_leave = flt(leave[0][3])

# 					equivalent_lwp_count = (1 - fraction_of_daily_salary_for_one_half_day) if is_one_half_day_leave else 1
...
#----------------------------------------
		...
		attendances = frappe.db.sql(
			...
			FROM `tabAttendance`
			WHERE
				status in ("Absent", "Half Day", "On leave", "Quarter Day", "Three Quarter Day",'One Quarter day','One Half Day')
				AND employee = %s
				AND docstatus = 1
		...
		for d in attendances:
			if (
				d.status in ("Half Day", "On Leave","Quarter Day", "Three Quarter Day",'One Quarter day','One Half Day')
				and d.leave_type
